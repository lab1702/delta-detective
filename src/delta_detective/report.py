...
{% if cfg.field_tolerances %}<p>Numeric field tolerances are active. Differences at or below each absolute tolerance count as unchanged, with exact matches and differences within tolerance reported separately. Null transitions still count as changes. Metric reconciliation and metric exports remain unchanged.</p>{% endif %}
<section><h2>Field-level changes: {{ bundle.field_changes.status }}</h2>
{% if bundle.field_changes.fields %}<p>{{ bundle.field_changes.changed_rows }} of {{ bundle.field_changes.matched_rows }} matched records changed in at least one selected field. Added and removed records are excluded. Field counts overlap; do not sum across fields. Blank means exactly empty text; null transitions and blank transitions can overlap. Increased/decreased means later/earlier for dates and times. Field values appear only in explicitly enabled transition summaries below.</p>
{% for f in bundle.field_changes.fields %}<h3 id="field-{{ loop.index0 }}">{{ f.name }} ({{ f.type }})</h3>{% if f.absolute_tolerance is not none %}<p>Absolute tolerance: {{ f.absolute_tolerance }}. Increased/decreased counts exclude differences within tolerance. Floating-point fields use approximate arithmetic.</p>{% endif %}<table><tr><th>Measurement</th><th>Records</th></tr>{% for key in ['changed_rows','unchanged_rows','exact_match_rows','within_tolerance_rows','became_null','from_null','both_null','value_changed_rows','became_blank','from_blank','increased_rows','decreased_rows'] if key in f %}<tr><td>{{ key }}</td><td>{{ f[key] }}</td></tr>{% endfor %}</table><p>Changed: {{ f.percent_changed|string + '%' if f.percent_changed is not none else 'undefined (no matched records)' }}.</p>
{% if f.transitions is defined %}<h4>Field transitions</h4><p>Enabled for this field: source and destination values are disclosed. Changed matched records only. {{ f.transitions.ranking }}</p>
{% if f.transitions.rows %}<table><tr><th>From</th><th>To</th><th>Records</th></tr>
{% for row in f.transitions.rows %}<tr>
<td>{% if row.is_other %}Other (remainder){% elif row.from_value is none %}NULL (missing value){% else %}Value: {{ row.from_value|tojson }}{% endif %}</td>
<td>{% if row.is_other %}Other (remainder){% elif row.to_value is none %}NULL (missing value){% else %}Value: {{ row.to_value|tojson }}{% endif %}</td>
<td>{{ row.rows }}</td></tr>{% endfor %}</table>
<p>{{ f.transitions.distinct_transitions }} distinct transitions; {{ f.transitions.omitted_transitions }} grouped into Other. Total: {{ f.transitions.changed_rows }} changed records. Evidence: {{ f.transitions.evidence }}.</p>
{% else %}<p>No changed matched records for this field.</p>{% endif %}{% endif %}
{% endfor %}{% endif %}</section>
