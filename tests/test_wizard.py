...
def run_wizard(tmp_path, answers, **kwargs):
    ...
    path = init_config(tmp_path/'reference.parquet', tmp_path/'current.parquet', tmp_path/'new.yaml',
                       ask=lambda prompt: '' if prompt.startswith(('Configure schema,', 'Configure input parsing,')) else next(answers), tell=messages.append, **kwargs)
...
def test_malformed_csv_does_not_expose_values(tmp_path):
    ...
    with pytest.raises(InvestigationError) as exc:
        init_config(tmp_path/'reference.csv', tmp_path/'current.csv', tmp_path/'new.yaml', ask=lambda p: '', tell=lambda s: None)
...
def test_concurrent_output_creation_is_not_overwritten(tmp_path):
    ...
    answers = iter(['1', '1', '', '', '', ''])
    def ask(prompt):
        if prompt.startswith(('Configure schema,', 'Configure input parsing,')):
            return ''
